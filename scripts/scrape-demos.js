#!/usr/bin/env node
/**
 * Scrapes https://crimson-ceremony.net/demopals/eurodemo/index.php
 * and builds a SQLite database of all Euro Demo disc variants.
 *
 * Schema:
 *   discs  - one row per regional variant (product_code + region)
 *   games  - playable/trailer/etc entries keyed by product_code
 *
 * Title  → from li.vartitle (first text line, before the catalog span)
 *          falls back to the h2 text when vartitle is empty
 * Region → from li.varflag img alt/title
 *          NULL when varflag is empty (disc applies to all regions)
 *
 * Images are downloaded to assets/disc-images/
 */
...
const PAGE_URL = `${BASE_URL}/demopals/eurodemo/index.php`;
...
// Extract the vartitle text — only the first line, before the <br> + catalog span.
function extractVarTitle(varTitleEl) {
  if (!varTitleEl) return null;
  // .text includes \n where <br> appears; take only what comes before it.
...
function parseDiscs(html) {
  ...
  const entries = []; // flat list: one entry per regional variant
  ...
  while (i < children.length) {
    ...
    // Advance to h3 then to div.demo
    let j = i + 1;
    ...
    // Games — shared across all regional variants of this product code
    const games = [];
    if (demoDiv) {
    ...
    // Regional variants — one <ul class="varitem"> per region
    const variants = [];
    if (demoDiv) {
      ...
      for (const ul of varLists) {
        ...
        // NULL region → disc applies to all regions
        const region = flagImg
          ? (flagImg.getAttribute('title') || flagImg.getAttribute('alt') || null)
          : null;

        // Title from vartitle first line; fall back to h2 text
        const title = extractVarTitle(varTitleEl) || h2Title;

        // Use -1.jpg (high-res front cover) instead of the -0.jpg thumbnail
    ...
    // If no variants at all, record a placeholder (no image) so games aren't lost
